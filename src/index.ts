...
// Store API key at server level
let serverApiKey: string | null = null;

async function transformTools(tools: ApiTool[]) {
  return tools
    .map((tool) => {
...
async function setupServerHandlers(server: Server, tools: ApiTool[]) {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      // Inject server-level API key if not provided
      const enhancedArgs = {
        ...args,
        apiKey: args.apiKey || serverApiKey
      };
      
      const result = await tool.function(enhancedArgs);
      return {
        content: [
          {
            type: "text",
...
async function run() {
  ...
  if (apiKeyArg) {
    ...
    if (apiKey) {
      serverApiKey = apiKey;
      console.error(`[Info] API key configured from command line`);
    }
  }
